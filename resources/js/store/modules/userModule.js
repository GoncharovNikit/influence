...
export const userModule = {
  ...
  mutations: {
    ...
    setUserPasswordConfirmation(state, password_c) {
      state.user.password_confirmation = password_c;
    },
    setUserData(state, user) {
      state.user = user;
    },
    setFetchedErrors(state, errors) {
      state.errors = errors;
    },
  ...
  actions: {
    async axiosErrorsHandler({}, error) {
      ...
      switch (error.response.status ?? -1) {
        ...
        default:
          console.log(
            "This status didn't processed. (" + error.status ?? -1 + ")"
          );
          break;
      }
    },
    async registerUser({ state, commit, dispatch }) {
      ...
      try {
        ...
        await axios
          .post("/api/auth/register", state.user)
          .then((response) => {
        ...
        await axios
          ...
          .then((response) => {
            ...
            if (response.status == 200) {
              ...
              document.cookie =
                "token=" + encodeURIComponent(response.data.access_token);
      ...
      try {
        ...
        await axios
          ...
          .then((response) => {
            ...
            if (response.status == 200) {
              console.log(
                "Profile has taken successfull. Setting to the state..."
              );
      ...
      try {
        ...
        await axios
          ...
          .then((response) => {
            ...
            if (response.status == 200) {
              console.log(
                "Token refreshed successfull. Setting new value to cookie..."
              );
              document.cookie =
                "token=" + encodeURIComponent(response.data.access_token);
            }
    ...
    auth_id() {
      try {
        axios.get("/api/auth_id").then((response) => {
          console.log(response);
        });
      } catch (error) {}
    },
